...
def solve():
    with open('testtubes.in', 'r') as f:  # Open the file for reading
        N, P = map(int, f.readline().split())  # Read the first line from the file
        tubes = [to_reduced_list(f.readline().strip()) for _ in range(2)]  # Read the next two lines for tube data
    tubes.append([])
...
T = 1  # Assuming there's only one test case in the file for simplicity
for _ in range(T):
    solve()
